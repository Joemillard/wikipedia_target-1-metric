...
average_monthly_views <- readRDS(here::here("data/class_wiki_indices/submission_2/user_trends/total_monthly_views_10-languages.rds"))
...
average_lambda <- function(data_file, taxa){
  ...
  data_subset <- data_fin[, (grep(1971, colnames(data_fin))):ncol(data_fin)]
...
for(i in 1:length(merge_fin_lambda)){
  avg_lambdas[[i]] <- lapply(merge_fin_lambda[[i]], average_lambda, taxa = classes[[i]])
}
...
ggsave("taxa_language_rate-of-change.png", scale = 1.1, dpi = 350)
